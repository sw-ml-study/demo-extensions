...
UTF-8 name/version slices and a bounded function descriptor array. Values use a
numeric tag plus an aligned payload union. Errors use a numeric code and message
slice. Reserved fields must be zero so V1 can reject accidental layout drift.
...
Validation immediately copies names, versions, and function metadata into
owned Rust values. Registration code must retain only those validated values,
not extension-owned pointers. The narrow pointer-reading implementation lives
in `validate.rs`; the rest of the ABI crate remains safe Rust.
