//! Minimal host for the MLPL tokenizer demo and static extension provider.
...
fn run() -> Result<(), String> {
    let descriptor = mlpl_extension_hftok::static_entry();
    ...
    let source = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../demos/hftok/roundtrip.mlpl"),
    )
